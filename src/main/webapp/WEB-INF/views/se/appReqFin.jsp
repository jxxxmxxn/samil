...
<main>
	...
	<select id="pageSelector" onchange="navigateToPage()">
		...
		<option value="/se/appReq">결재 대기함</option>
    </select>
	</div>
	<table class="tg">
		<tr>
		...
		<c:forEach var="requestAppFin" items="${requestFinList}">
			<tr>
				...
				<td>
					<fmt:formatDate value="${requestAppFin.approvalDate}" pattern="yy/MM/dd" />
				</td>
		        <td class="${requestAppFin.approvalCondition == 130 ? 'status-red' : 
	                         requestAppFin.approvalCondition == 120 ? 'status-green' : 
	                         requestAppFin.approvalCondition == 110 ? 'status-yellow' : 
	                         requestAppFin.approvalCondition == 100 ? 'status-white' : ''}"
						style="padding: 8px; display: inline-block; margin-top: 10px;">
		                ${statusMap[requestAppFin.approvalCondition]}
	            </td>
				<td>
